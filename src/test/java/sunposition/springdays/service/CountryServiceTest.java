...
class CountryServiceTest {
    ...
    /*@Test
    void testUpdateCountryByName_CountryAlreadyExists() {
        when(repositoryOfCountry.findByName("OldName")).thenReturn(Optional.of(new Country()));
        when(repositoryOfCountry.findByName("NewName")).thenReturn(Optional.of(new Country()));

        try {
            countryService.updateCountryByName("OldName", "NewName");
            fail("Expected CustomBadRequestException to be thrown");
        } catch (HttpErrorExceptions.CustomBadRequestException e) {
            assertEquals("Country with the same name already exists", e.getMessage());
        } catch (Exception e) {
            e.printStackTrace();
